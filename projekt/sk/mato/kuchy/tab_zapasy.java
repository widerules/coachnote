...
protected void onPause() {
	...
	/*try {
		xml.zapisHracomNovyrespekt( trening.getHracov() );
		Toast.makeText(getBaseContext(), "novy hraci ulozeny!"
...
private boolean vypisuj(){
      ...
      //pozor mozna chyba pri deletovani hraca co uz ma zapas to urcite spadne!
        
		if (trening.getZapasy().size()<1) trening.generujZapasy();
	    ...
	    for (Zapas zapas : zapasy) {
	        ...
	        tv.setText(zapas.getA().getMeno()+" "+zapas.getA().getPriezvisko()+" vs. "+zapas.getB().getMeno()+" "+ zapas.getB().getPriezvisko());
	        linLay.addView(tv);
	        nadpisy.add(tv);
	    	...
	    	ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(
	    		    this, R.array.zapas_array, android.R.layout.simple_spinner_item);
	  ...
	  //zberdat
	   ...
	   RelativeLayout.LayoutParams.FILL_PARENT, 
	    ...
	    odosli.setOnClickListener(new OnClickListener() {//totok nefunguje!
			...
			public void onClick(View v) {
				...
				for (Zapas zapas : zapisuj) {
					...
					if ( vytazy.get(2*i).isChecked() ) zapas.setVytaz(zapas.getA());
					if ( vytazy.get(2*i+1).isChecked() ) zapas.setVytaz(zapas.getB());
					int vysledok=-1;
					vysledok= spinnery.get(i).getSelectedItemPosition()-1;
				...
				try {
					...
					Toast.makeText(getBaseContext(), "Zapasy, vysledky ulozene!"
							, Toast.LENGTH_LONG).show();
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
					Toast.makeText(getBaseContext(), "Zapasy, vysledky NEulozene!"
							, Toast.LENGTH_LONG).show();
				}
	    ...
	    //linLay.addView(odosli);
	    butony.addView(odosli);
	    //linLay.addView(butony);
	    //linLay.addView(butony);
