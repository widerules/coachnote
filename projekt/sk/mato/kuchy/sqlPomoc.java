...
public class sqlPomoc extends SQLiteOpenHelper {
	...
	public Hrac getHraca(String meno, String priezvisko) {
		...
		Hrac novy;
		cursor.moveToFirst();
		novy = new Hrac(Integer.parseInt(cursor.getString(0)),cursor.getString(1), cursor.getString(2), Integer
				.parseInt(cursor.getString(3)), Double.parseDouble(cursor
				.getString(4)));
		cursor.close();
		db.close();
		return novy;
	...
	public int getIdHraca(String meno, String priezvisko) {
		SQLiteDatabase db = this.getReadableDatabase();
		Cursor cursor = db.rawQuery("SELECT id FROM `hraci` WHERE `meno` LIKE '"
				+ meno + "' AND `priezvisko` LIKE '" + priezvisko + "' ",
				new String[] {});
		cursor.moveToFirst();
...
//chyba doiplementovat iny typ...
		...
		Cursor cursor = db.rawQuery("SELECT id FROM `zapasy` WHERE `typ` = '"
				+ typ + "' AND `teamA` = '" + teamA + "' AND `teamB` = '" + teamB+ "' AND `datum` = '" + z.getDatum().toString() + "' ",
				new String[] {});
		cursor.moveToFirst();
	...
	public ArrayList<Hrac> dajCeluDb() {
		...
		for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
			novy.add(new Hrac(Integer.parseInt(cursor.getString(0)),cursor.getString(1), cursor.getString(2), Integer
					.parseInt(cursor.getString(3)), Double.parseDouble(cursor
					.getString(4))));
		}
		cursor.close();
