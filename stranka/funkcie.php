...
FROM `treningy_temp` ";
   	 ...
   	 /*	$max= getMaxZa();
		 ...
		 $teamB= explode("/",$row['zapasy']);
				foreach ($teamB as &$hrac) {
					if ($hrac>0){//v podstate null check
						$hrac=$max+$hrac;
						}  
						
					}
				$row['zapasy']= implode("/", $teamB);				
				unset($hrac);  	 	
   	 	*/
...
';
	...
	function vypisTreningyKlubu($id) {
		echo '<h2> vsetky treng nasho klubu: </h2>';
			if ($link = prihlas_sql()) {
//to id klubu sa bude p[ouzivat na selectnutie tabulky ale teraz to bezi iba jeden klub
	$sql = '
SELECT datum as d, id, hraci
FROM `treningy`
';
